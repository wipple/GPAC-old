	...
	case DOWNLOAD_SERVER_INITIATE:

		{
			...
			if(DownloadServerInit->privateDataLength){
				...
				GroupInfoIndic->NumberOfGroups = gf_bs_read_int(bs,16);
				if(GroupInfoIndic->NumberOfGroups){
					GroupInfoIndic->InfoGroup = (GF_M2TS_DSMCC_INFO_GROUP*)gf_calloc(GroupInfoIndic->NumberOfGroups,sizeof(GF_M2TS_DSMCC_INFO_GROUP));
					for(i = 0;i<GroupInfoIndic->NumberOfGroups ;i++){
						GroupInfoIndic->InfoGroup[i].GroupId =  gf_bs_read_int(bs,32);
						GroupInfoIndic->InfoGroup[i].GroupSize =  gf_bs_read_int(bs,32);
						gf_m2ts_dsmcc_process_compatibility_descriptor(&GroupInfoIndic->InfoGroup[i].CompatibilityDescr,data,bs,data_shift);
						GroupInfoIndic->InfoGroup[i].GroupInfoLength =  gf_bs_read_int(bs,16);
						if(GroupInfoIndic->InfoGroup[i].GroupInfoLength){
							GroupInfoIndic->InfoGroup[i].groupInfoByte = (char*)gf_calloc(GroupInfoIndic->InfoGroup[i].GroupInfoLength,sizeof(char));
							gf_bs_read_data(bs,GroupInfoIndic->InfoGroup[i].groupInfoByte,(u32)(GroupInfoIndic->InfoGroup[i].GroupInfoLength));
						}
					}
				}
				GroupInfoIndic->PrivateDataLength =  gf_bs_read_int(bs,16);
				if(GroupInfoIndic->PrivateDataLength){
					GroupInfoIndic->privateDataByte = (char*)gf_calloc(GroupInfoIndic->PrivateDataLength,sizeof(char));
					gf_bs_read_data(bs,GroupInfoIndic->privateDataByte,(u32)(GroupInfoIndic->PrivateDataLength));
				}
			}
			localbyteshift = ((u32)(gf_bs_get_position(bs)) - localbyteshift);
			if( localbyteshift == DataMessage->DownloadDataHeader.messageLength){
				GF_LOG(GF_LOG_INFO, GF_LOG_CONTAINER, ("[Process DSMCC] DSMCC processed length error. Difference between byte shifting %d and data size %d \n",((u32)(gf_bs_get_position(bs)) - localbyteshift),DataMessage->DownloadDataHeader.messageLength));
			}*/
			break;
		}
